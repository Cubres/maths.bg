...
import IntegrationDetailModal from "./integrations/components/IntegrationDetailModal"
import LoadingScreen from "./components/LoadingScreen"
import AnnouncementBanner from "./components/AnnouncementBanner"
import CollectionDialog from "./integrations/components/CollectionDialog"
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Check if we have cached data or if this is internal navigation
    const cachedData = localStorage.getItem("problems-cache-timestamp")
    const isNavigatingInternally = sessionStorage.getItem("internal-navigation") === "true"

    if ((cachedData && showLoadingScreen) || isNavigatingInternally) {
      // If we have cached data or navigating internally, skip the loading screen
      // setLoadedProblems(totalProblems);
      // loadingCompleted.current = true;
      setIsInternalNavigation(isNavigatingInternally)
  ...
  return (
    <>
      {/* Add ThemePersistence component to ensure theme is maintained */}
      <ThemePersistence />

      {showLoadingScreen && !isInternalNavigation && (
        <LoadingScreen totalProblems={integrations.length} onComplete={() => setShowLoadingScreen(false)} />
      )}

      <div
        className="flex h-screen bg-gray-100 dark:bg-[#171717] overflow-hidden max-w-[100vw]"
      ...
      >
        <CategoryFilter
          ...
          onSelectCollection={(collectionId) => {
            ...
            window.history.pushState(null, "", `/collections/${collectionId}`)
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          <div className="p-4 md:p-6 space-y-4">
            <div className="flex justify-between items-center">
              {/* Pass filteredIntegrations and onSelectIntegration to SiteHeader */}
              <SiteHeader
                contentType={contentType}
